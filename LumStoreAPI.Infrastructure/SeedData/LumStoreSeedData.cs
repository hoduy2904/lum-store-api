...
namespace LumStoreAPI.Infrastructure.SeedData
{
    public static class LumStoreSeedData
    {
        ...
        private static async Task SeedDocumentTreeAsync(LumStoreContext context)
        {
            ...
            var root = new DocumentNode { NodeAlias = "root", NodeOrder = 1 };
            await context.DocumentNodes.AddAsync(root);
            await context.SaveChangesAsync();

            // ── Level 1: Main pages + 6 categories directly under root ─────────

            var home       = new DocumentNode { NodeAlias = "home",       NodeOrder = 1, ParentNodeID = root.NodeID };
            var catGel     = new DocumentNode { NodeAlias = "gel-polish", NodeOrder = 2, ParentNodeID = root.NodeID };
            var catNailArt = new DocumentNode { NodeAlias = "nail-art",   NodeOrder = 3, ParentNodeID = root.NodeID };
            var catBundles = new DocumentNode { NodeAlias = "bundles",    NodeOrder = 4, ParentNodeID = root.NodeID };
            var catTools   = new DocumentNode { NodeAlias = "tools",      NodeOrder = 5, ParentNodeID = root.NodeID };
            var catCare    = new DocumentNode { NodeAlias = "care",       NodeOrder = 6, ParentNodeID = root.NodeID };
            var catHoliday = new DocumentNode { NodeAlias = "holiday",    NodeOrder = 7, ParentNodeID = root.NodeID };
            ...
            var nodeBlackWhite = new DocumentNode { NodeAlias = "black-white-gel-duo",         NodeOrder = 1, ParentNodeID = catGel.NodeID };
            var nodeNude       = new DocumentNode { NodeAlias = "nude-collection-gel-polish",   NodeOrder = 2, ParentNodeID = catGel.NodeID };
            var nodeNeon       = new DocumentNode { NodeAlias = "neon-summer-collection",       NodeOrder = 3, ParentNodeID = catGel.NodeID };
            var nodeTopCoat    = new DocumentNode { NodeAlias = "gel-top-coat-shiny",           NodeOrder = 4, ParentNodeID = catGel.NodeID };

            // Nail Art — 1 product
            var nodeChrome     = new DocumentNode { NodeAlias = "chrome-powder-kit",            NodeOrder = 1, ParentNodeID = catNailArt.NodeID };

            // Bundles — 2 products
            var nodeHalloween  = new DocumentNode { NodeAlias = "v2-halloween-bundle",          NodeOrder = 1, ParentNodeID = catBundles.NodeID };
            var nodeStarterKit = new DocumentNode { NodeAlias = "gel-starter-kit",              NodeOrder = 2, ParentNodeID = catBundles.NodeID };

            // Tools — 2 products
            var nodeBrushSet   = new DocumentNode { NodeAlias = "precision-nail-art-brush-set", NodeOrder = 1, ParentNodeID = catTools.NodeID };
            var nodeFileSet    = new DocumentNode { NodeAlias = "nail-file-buffer-set",         NodeOrder = 2, ParentNodeID = catTools.NodeID };

            // Care — 2 products
            var nodeCuticle    = new DocumentNode { NodeAlias = "cuticle-oil-trio",             NodeOrder = 1, ParentNodeID = catCare.NodeID };
            var nodeHandCream  = new DocumentNode { NodeAlias = "hand-cream-luxury",            NodeOrder = 2, ParentNodeID = catCare.NodeID };

            // Holiday — 1 product
            var nodeHoliday    = new DocumentNode { NodeAlias = "holiday-bundle",               NodeOrder = 1, ParentNodeID = catHoliday.NodeID };
            ...
            await context.DocumentPages.AddAsync(
                new DocumentPage { NodeID = root.NodeID, DocumentName = "Root", ClassName = "CMS.Folder" }
            );
            ...
            await context.ProductCategories.AddRangeAsync(
                new ProductCategory
                {
                    ...
                    ClassName           = ProductCategory.CLASS_NAME,
